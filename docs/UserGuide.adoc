...
=== Save Contacts To Current User: `save` - Since V1.5

Save the contacts to the current user. +
Format: `save`

****
* Use this `SAVE` command when performing a mutating command, i.e. a command that changes the data in the storage files
* e.g. add, delete, edit, remark, group ...
* Till this point, most of the mutating commands, including the `REMOVE USER`
command that releases the contacts to the `PUBLIC`, user are incorporated with this `SAVE` command.
****
// tag::order[]
